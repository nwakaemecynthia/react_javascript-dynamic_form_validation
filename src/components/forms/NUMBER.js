...
export const NUMBER = ({divClass, inputClass, formControl, formHook}) => {
			...
			switch(key) {
				...
				case 'maximum':
					setValidatorSchema(prevState => ({
						...prevState,
						max: {value, message: `Maximum of ${value} is required.`}
					}));
					break;
				case 'minimum':
					if(value) setValidatorSchema(prevState => ({
						...prevState,
						min: {value, message: `Minimum of ${value} is required.`}
					}));
					break;
				default:
					break;
	...
	return (
		<div className={divClass}>
			...
			<input className={inputClass}
						 ...
						 max={formControl["maximum"]}
						 min={formControl["minimum"]}
